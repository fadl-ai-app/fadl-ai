...
def add_text_to_video_ui():
    ...
    text_video_check_button = gr.Button(
        "تحديث داخلي",
        visible=False,
        elem_id="fadl-text-video-check"
    )

    gr.HTML("""
    <script>
    (function () {
        if (window.__fadlTextVideoPoll) return;
        window.__fadlTextVideoPoll = true;

        setInterval(function () {

            const stateRoot =
                document.getElementById(
                    "fadl-text-video-state"
                );

            if (!stateRoot) return;

            const stateInput =
                stateRoot.querySelector(
                    "textarea, input"
                );
            ...
            const root =
                document.getElementById(
                    "fadl-text-video-check"
                );

            if (!root) return;

            const button =
                root.querySelector("button") || root;
    ...
    text_video_check_button.click(
        fn=check_text_video_status,
...
def check_text_video_status(state_json):
    ...
    if task_status == "SUCCEEDED":
        ...
        if not outputs:
            return (
                "✗ اكتمل التوليد لكن تعذر استلام الفيديو",
                None,
                None,
                state_json
            )
...
def confirm_and_generate_video(confirmation_json):
    ...
    # لا نحجز واجهة المستخدم أثناء انتظار الفيديو
    return (
