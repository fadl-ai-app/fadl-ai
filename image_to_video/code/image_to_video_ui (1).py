...
def add_image_to_video_ui():
    ...
    check_video_button = gr.Button(
        "🔄 تحديث حالة الفيديو",
        visible=False,
        elem_id="fadl-auto-video-check"
    )

    gr.HTML("""
    <script>
    (function () {
        if (window.__fadlVideoAutoPoll) return;
        window.__fadlVideoAutoPoll = true;

        setInterval(function () {
            const stateRoot =
                document.getElementById("fadl-video-state");

            if (!stateRoot) return;

            const stateInput =
                stateRoot.querySelector("textarea, input");
            ...
            const buttonRoot =
                document.getElementById(
                    "fadl-auto-video-check"
                );

            if (!buttonRoot) return;

            const button =
                buttonRoot.querySelector("button")
                || buttonRoot;
    ...
    check_video_button.click(
        fn=check_image_generation_status,
        inputs=confirmation_data,
        outputs=[
            pipeline_status,
            final_video,
...
def check_image_generation_status(state_json):
    ...
    try:
        ...
        return (
            "✗ بيانات متابعة الفيديو غير صالحة",
            None,
            None,
            ""
        )
    ...
    try:
        ...
        return (
            "⏳ جاري إنشاء الفيديو. حاولي التحديث بعد قليل.",
            None,
            None,
            state_json
        )
    ...
    if r.status_code != 200:
        return (
            "⏳ جاري متابعة الفيديو. حاولي بعد قليل.",
            None,
            None,
            state_json
        )
    ...
    if task_status == "SUCCEEDED":
        ...
        if not outputs:
            return (
                "✗ اكتمل التوليد لكن لم يظهر ملف الفيديو",
                None,
                None,
                state_json
            )
        ...
        return (
            "✅ تم إنشاء الفيديو بنجاح وهو جاهز للعرض والتحميل.",
            gr.update(
                value=str(watermarked_path),
                visible=True
            ),
            gr.update(
                value=str(watermarked_path),
    ...
    return (
        "⏳ الفيديو ما زال قيد الإنشاء. اضغطي تحديث بعد قليل.",
        None,
        None,
        state_json
    )
...
def confirm_image_generation(confirmation_json):
    ...
    # =====================================================
    # عدم حجز واجهة المستخدم أثناء انتظار Runway
    # =====================================================

    return (
        "⏳ جاري إنشاء الفيديو… سيظهر تلقائيًا عند اكتماله.",
        None,
        None,
        json.dumps(
            {
